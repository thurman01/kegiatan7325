  ...
  reader.onload = function (e) {
    ...
    img.onload = function () {
      ...
      navigator.geolocation.getCurrentPosition(pos => {
        ...
        ctx.fillText(`Lokasi: ${loc}`, 10, 25);
        ctx.fillText(`Waktu: ${waktu}`, 10, 50);
      });
    };
...
async function submitForm() {
  ...
  const bidang = bidangRadio.value === "LAINNYA" ? document.getElementById("bidang-lain").value : bidangRadio.value;
  const waktu = new Date().toLocaleString();
  const canvas = document.getElementById("canvas-foto");
  ...
  const upload = await fetch("https://script.google.com/macros/s/AKfycbyiMChjTq54ovT7ID7lMZUHp_dOCBDl7DAwGWfF8h_UoT50qwwi20woon1ZU41HGMWotA/exec", {
    method: "POST",
    body: JSON.stringify({ mode: "upload", user: usernameGlobal, kegiatan, bidang, waktu, filename, foto: base64Image }),
  });
